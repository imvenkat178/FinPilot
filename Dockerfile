...
# Uvicorn believes X-Forwarded-For only from these peers. Override it with the ingress address,
# or the IP-keyed sign-up and sign-in throttles count every user together (G1.2).
...
CMD ["python", "-m", "uvicorn", "finpilot.api.app:app", "--host", "0.0.0.0", "--port", "8000", "--no-access-log"]
