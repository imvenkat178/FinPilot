...
The adapter requests Transactions, with Liabilities as an optional product, and imports checking, savings, money-market, credit card, auto loan, personal loan, mortgage and student loan accounts. Every linked debt keeps its balance in debt totals and net worth. Card and loan terms come only from Plaid Liabilities: purchase APR, minimum payment, due date and statement details for cards, and rate, minimum payment and due date for student loans. A debt whose rate or required payment was not reported is marked `terms_complete = false`, so payoff comparisons list it under `missing_terms` instead of modelling it at a 0% rate. A servicer's mortgage payment can include escrow, which FinPilot keeps out of the interest model, so a linked mortgage waits until someone enters principal and interest and escrow; later syncs update its rate and due day but keep that split. A card record is created only when the limit, statement date, due date and purchase APR are all reported. Missing current/available balances are not replaced with invented values. Previously imported accounts whose current snapshot is missing or incomplete retain their last recorded balances with stale provenance and an unhealthy connection.
...
Bank categories map onto one FinPilot vocabulary in `finpilot/integrations/categories.py`, such as `groceries`, `dining`, `gas`, `utilities` and `debt_payment`, so card rewards, bills and chat filters treat bank data like manual data. Plaid's detailed label is kept as `provider_category`. Card payments, loan payments, transfers and interest charges get their own transaction kinds, so they never count as income or ordinary spending. A category the user corrected stays corrected on later syncs. CSV import is refused for an account that syncs from a healthy bank connection, because the same activity would be imported twice; it works again after Disconnect.

Another aggregator such as MX or Yodlee would plug in at the same seams: the account, liability and transaction apply functions, the normalized terms that `fetch_liabilities` returns, and a category table. No second adapter exists; choosing one is roadmap decision G8.9.
...
## Figures, evidence and topic checks in answers

Model wording never supplies a number. `finpilot/ai/facts.py` replaces every figure in the deterministic reference answer with a token such as `[F1]`: amounts, dates, percentages, bare numbers, number words and lone currency symbols. The question, conversation context and document excerpts reach the model with their figures masked. The server puts the exact figures back and rejects a draft that uses a token that does not exist, types a digit, currency symbol, percent sign or number word itself, or drops every figure. Numeric grounding, caveat, action-claim and spending-allowance checks then run on the restored text, followed by an added-advice check: wording that recommends buying or selling investments, opening or refinancing a product, or declares eligibility or legality is rejected unless the reference answer already says it. Any rejection falls back to the calculator answer and records the reason in the trace. Tokens make every figure exact, but they do not prove that a sentence gives each figure its correct role; only the spending allowance has a dedicated role check.

A deterministic topic gate (`in_domain` in `finpilot/ai/guardrails.py`) answers clearly unrelated requests with a fixed reply before planning or any model call. Its lexicon is deliberately broad, so a false match only reaches the normal deterministic path. Product-boundary refusals, follow-ups, document and memory questions, pasted context and structured workflow input skip the gate. Router requests with no recognized intent get fixed guidance without model wording.

Every `/api/ask` answer carries three fields built by `attach_evidence` in `finpilot/ai/evidence.py`:

- `record_refs`: links to the household records named in the results behind the answer: accounts, bills, cards, debts, goals, rules, income sources and transactions. An ID becomes a link only when that record exists in the household. Spending allowance and forecast results list the bills, income sources and pending transactions dated in their window, the allowance also lists its protected reserves, and transaction searches list each transaction.
- `figures`: for each figure in the answer, the result fields that hold exactly that value.
- `evidence_confidence`: `high`, `medium` or `low`, with a 0 to 100 score and the reason for each deduction. The rubric is in `docs/calculation-methods.md`; it is not the model's opinion of itself.

The three fields are validated against a JSON Schema before the answer is saved or returned. Output that fails is dropped rather than repaired, and the trace records why. Document answers keep their quoted-excerpt citations.

An in-process cache (`finpilot/ai/wording_cache.py`) keeps model wording that passed every check, and validated workflow plans, keyed by a digest of the household, model, prompt version and exact prompt. Cached wording holds tokens rather than figures, so a repeated question reuses it with the current figures and runs every check again; a cached plan is validated again on every use. Entries expire after 24 hours, each process keeps at most 512, and financial results are never cached. The match is exact, not similarity-based: two prompts share an entry only when their tokenized text is identical.
...
Requests include a request ID and `Server-Timing` header. Each request writes one line to the `finpilot.requests` logger on standard error with the method, path, status, duration and request ID. Request logs exclude bodies, questions, credentials, amounts, and query strings. Amounts and search text travel in JSON request bodies rather than URLs, because proxy and hosting platform logs record URLs. The container image turns off the Uvicorn access log, and when that log is on, FinPilot removes query strings from it. `tests/test_request_logging.py` checks both server commands and fails when a query parameter could carry an amount or free text. Audit records capture financial state changes separately from operational logs.
