...
| Owner's live roadmap page | On the owner's machine, a Claude Code Stop hook runs `scripts/roadmap_artifact.py stop-hook` and asks the session to republish a private claude.ai page built from `ROADMAP.md` and `HANDOFF.md` whenever either changed. Other agents only need to keep both files current |
...
| `docs/user-guide.md` | How to use FinPilot | A user-visible workflow changes |
| `docs/calculation-methods.md` | Formulas, defaults and limits of every engine | An engine's method, default or limit changes |
| `docs/configuration.md` | Every environment variable | Code reads a new setting or a default changes |
| `docs/frontend.md` | Web module map and conventions | The frontend structure or conventions change |
| `docs/api-reference.md`, `docs/api/openapi.json`, `docs/ai-capabilities.md`, `docs/data-model.md` | Generated references | Never by hand; run `scripts/build_docs.py` after changing routes, `finpilot/api/route_docs.py`, capabilities or models |
| `SECURITY.md`, `CONTRIBUTING.md`, `CHANGELOG.md` | Vulnerability reporting, contributor steps and change history | The process changes, or with each notable change |
